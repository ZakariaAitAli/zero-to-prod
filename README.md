...
register a new task-definition revision using that existing image
    ↓
deploy + externally verify the known-good version
    ↓
...
Rollback is workflow-assisted but not automatic. A failed deployment is cleaned up first; an operator decides whether to dispatch the rollback workflow.
